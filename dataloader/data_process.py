...
def read_network(path):
    ...
    rel_index = np.array(rel_index)

    # ✅ 正确计算节点数：最大节点ID + 1
    if len(edge_index) > 0:
        max_node_id = int(edge_index.max())
        num_node = max_node_id + 1  # 索引从 0 开始
    else:
        num_node = 0

    # ✅ 正确计算关系数
    if len(rel_index) > 0:
        num_rel = int(rel_index.max()) + 1
        unique_rel_count = len(set(rel_index))
    else:
        num_rel = 0
        unique_rel_count = 0

    print(f"[read_network] 统计信息:")
    print(f"  - 边数量: {len(edge_index)}")
    print(f"  - 最大节点ID: {max_node_id if len(edge_index) > 0 else 'N/A'}")
    print(f"  - num_node: {num_node} (节点索引范围: [0, {num_node - 1}])")
    print(f"  - 唯一关系数: {unique_rel_count}")
...
def generate_node_subgraphs(dataset, drug_id, network_edge_index, network_rel_index, num_rel, args):
    ...
    paths = "datasets/" + str(dataset) + "/" + str(method) + "/"
    ...
    if method == "khop-subtree":
        ...
        pagerank_paths = "datasets/" + str(dataset) + "/" + str(method) + "/pageRank.json"
...
def subtreeExtractor(drug_id, edge_index, rel_index, shortest_paths, num_rel, fixed_num, khop):
    ...
    # 缓存文件路径（pkl格式）
    pkl_path = os.path.join(shortest_paths, f"subtree_fixed_{fixed_num}_hop_{khop}sp.pkl")

    # 如果缓存文件已存在，则直接加载
    if os.path.exists(pkl_path):
        print(f"加载缓存子图文件: {pkl_path}")
        ...
        print(f"子图加载完成 | max_rel={max_rel:.0f}, max_degree={max_degree:.0f}")
        return subgraphs, max_degree, max_rel

    print(f"未检测到缓存文件，正在生成子图并保存到 {pkl_path} ...")
...
def rwExtractor(drug_id, edge_index, rel_index, shortest_paths, num_rel, sub_num, length):
    ...
    #             max_degree = subgraphs[s][7] if subgraphs[s][7] > max_degree else max_degree
    #     return subgraphs, max_degree, max_rel;
    pkl_path = shortest_paths + f"rw_num_{sub_num}_length_{length}sp.pkl"
    if os.path.exists(pkl_path):
        with open(pkl_path, 'rb') as f:
            subgraphs, max_degree, max_rel = pickle.load(f)
        return subgraphs, max_degree, max_rel
    ...
    # return subgraphs, max(max_degree), max(num_rel_update)
    with open(pkl_path, 'wb') as f:
        pickle.dump((subgraphs, max(max_degree), max(num_rel_update)), f)
    return subgraphs, max(max_degree), max(num_rel_update)
